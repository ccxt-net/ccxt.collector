...
namespace CCXT.Collector.Bithumb
{
    public class Pushing
    {
        ...
        private async ValueTask Open(CancellationToken cancelToken, ClientWebSocket cws, string symbol, bool reopen = true)
        {
        ...
        {
            BTLogger.SNG.WriteO(this, $"pushing service start: symbol => {symbol}...");
            ...
            using (var _cws = new ClientWebSocket())
            {
                var _sending = Task.Run(async () =>
                {
                    while (true)
                    {
                        try
                        {
                            ...
                            if (_waiting_time > __btconfig.WebSocketRetry * 1000)
                            {
                                __last_receive_time = CUnixTime.NowMilli;
                                await Open(cancelToken, _cws, symbol);

                                BTLogger.SNG.WriteO(this, $"pushing open: symbol => {symbol}...");
                            }
                        ...
                        //finally
                        {
                            ...
                            {
                                BTLogger.SNG.WriteO(this, $"disconnect from server(cmd): symbol => {symbol}...");
                                //cancelToken.Cancel();
                                break;
                            }
                    ...
                    while (true)
                    {
                        try
                        {
                            ...
                            else if (_result.MessageType == WebSocketMessageType.Binary)
                            {
                                ...
                                { 
                                    command = "WS",
                                    exchange = BTLogger.SNG.exchange_name,
                                    symbol = symbol,
                            ...
                            else if (_result.MessageType == WebSocketMessageType.Close)
                            {
                                ...
                                BTLogger.SNG.WriteO(this, $"receive close message from server: symbol => {symbol}...");
                                //cancelToken.Cancel();
                                break;
                            }
                        }
                        ...
                        //finally
                        {
                            if (_cws.State != WebSocketState.Open)
                            {
                                BTLogger.SNG.WriteO(this, $"disconnect from server: symbol => {symbol}...");
                                //cancelToken.Cancel();
                                break;
                            }
                ...
                BTLogger.SNG.WriteO(this, $"pushing service stopped: symbol => {symbol}...");
            }
        }
    }
}